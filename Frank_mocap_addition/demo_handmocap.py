...
#        print(pred_output_list[0]['left_hand']['mano_parameter'])
...
'''
def run_hand_mocap(args, bbox_detector, hand_mocap):
    #Set up input data (images or webcam)
    input_type, input_data = demo_utils.setup_input(args)
    ...
    while True:
        ...
        pred_output_list = hand_mocap.regress(
                img_original_bgr, hand_bbox_list, add_margin=True)
        Pre_hand_lists.append(pred_output_list)
...
'''     
def pic_2_mano(input_path='/media/sdc/haosheng23/baseline/howbullet/Frankmocap/sample_data/picture_test/',output_path='/media/sdc/haosheng23/baseline/howbullet/Frankmocap/mocap_output/',view_type='ego_centric'):
    args = DemoOptions().parse([])
    #path_current=os.getcwd() 
    path_current="/media/sdc/haosheng23/baseline/howbullet/Frankmocap"
    args.checkpoint_body_smpl=path_current+'/extra_data/body_module/pretrained_weights/2020_05_31-00_50_43-best-51.749683916568756.pt'
    args.checkpoint_body_smplx=path_current+'/extra_data/body_module/pretrained_weights/smplx-03-28-46060-w_spin_mlc3d_46582-2089_2020_03_28-21_56_16.pt'
    args.checkpoint_hand=path_current+"/extra_data/hand_module/pretrained_weights/pose_shape_best.pth"
    args.smpl_dir=path_current+'/extra_data/smpl/'
    args.use_smplx = True
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    assert torch.cuda.is_available(), "Current version only supports GPU"
    ...
    pred_output_list=run_hand_mocap(args, bbox_detector, hand_mocap)
