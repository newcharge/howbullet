...
class ARCTICHelper:
    def __init__(self, data_dir, split, use_cam_coordinate=False):  # sequences are named such as s%d/%s
        self.data_dir = data_dir  # Usually named as ${any_dirname}/splits
        self.split = split
        self.file_path = os.path.join(data_dir, f"{split}.npy")
        self.use_cam_coordinate = use_cam_coordinate
    ...
    def prepare_data(self):
        ...
        param_name = "cam_coord" if self.use_cam_coordinate else "world_coord"
        for seq in data["data_dict"]:
            joints_data = data["data_dict"][seq][param_name]["joints.right"]
            if self.use_cam_coordinate:
                for i in range(joints_data.shape[1]):
                    joints3d.append(joints_data[:, i, ...])
                    all_frame_ids.append(range(joints_data.shape[0]))
            else:
                joints3d.append(joints_data[:])
                all_frame_ids.append(range(joints_data.shape[0]))
